...
class CLAP_Module(torch.nn.Module):
    def __init__(
        ...
        target_audio_layer_name: Optional[str]
            Name of the audio encoder layer to extract features from (relative to audio_branch).
            E.g., 'encoder.blocks.11' for an HTSAT model.
            If None, the final embedding is used.
        """
        ...
        # Call create_model without the new argument first
        if enable_fusion:
            fusion_type = "aff_2d"
            model, model_cfg = create_model(
                ...
                # target_audio_layer_name=target_audio_layer_name, # Removed from here
            )
        else:
            model, model_cfg = create_model(
                amodel,
        ...
        # Now, if target_audio_layer_name is provided, set it on the model
        # and attempt to register the hook.
        # This relies on the CLAP model (in model.py) having these attributes/methods.
        if target_audio_layer_name:
            if hasattr(self.model, "target_audio_layer_name"):
                self.model.target_audio_layer_name = target_audio_layer_name
                if hasattr(self.model, "_register_audio_hook"):
                    self.model._register_audio_hook()
                    logging.info(
                        f"CLAP_Module: Set target_audio_layer_name and called _register_audio_hook on underlying model for layer: {target_audio_layer_name}"
                    )
                else:
                    logging.warning(
                        "CLAP_Module: Underlying CLAP model does not have _register_audio_hook method. Intermediate features may not be extracted."
                    )
            else:
                logging.warning(
                    "CLAP_Module: Underlying CLAP model does not have target_audio_layer_name attribute. Intermediate features may not be extracted."
                )

        self.enable_fusion = enable_fusion  # This was correctly placed
    ...
    def load_ckpt(self, ckpt=None, model_id=-1, verbose=True):
        ...
        if ckpt is not None:
            ...
            if os.path.exists(ckpt):
                ...
                ckpt = wget.download(
                    download_link + weight_file_name, os.path.dirname(ckpt)
                )
