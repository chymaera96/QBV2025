...
def extract_control_features(waveform, cfg, feature_len):
    ...
    pitch = librosa.yin(waveform_np, fmin=50.0, fmax=1100.0, sr=sample_rate, hop_length=hop_length)
    ...
    pitch = torch.tensor(scipy.signal.medfilt(pitch, kernel_size=5))

    # Stack and interpolate
    features = torch.stack([loudness, centroid, pitch])  # [3, T]
...
class AESAIMLA_DEV(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        ref_feat = extract_control_features(ref_audio, self.cfg, self.feature_len)
        query_feat = extract_control_features(query_audio, self.cfg, self.feature_len)
