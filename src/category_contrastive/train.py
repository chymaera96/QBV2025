...
class CategoryContrastiveLoss(nn.Module):
    ...
    def forward(self, query_embeddings, reference_embeddings, category_indices):
        ...
        # For each query, compute log-sum-exp of positive similarities
        exp_similarities = torch.exp(similarity_matrix)

        # Sum of positive similarities for each query
        positive_sums = torch.sum(exp_similarities * positive_mask, dim=1)

        # Sum of all similarities for each query
        all_sums = torch.sum(exp_similarities, dim=1)

        # Avoid log(0) by ensuring positive_sums > 0
        positive_sums = torch.clamp(positive_sums, min=self.eps)

        # Supervised contrastive loss
        loss = -torch.log(positive_sums / all_sums).mean()
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        # Category-wise contrastive loss
        loss = criterion(query_emb, reference_emb, category_indices)
